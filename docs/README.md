...
| [meshtastic.md](meshtastic.md) | **Can one radio be an XPRS station and a Meshtastic node at once?** A LoRa receiver is configured for a modulation, not a frequency, and the sync word is compared in silicon before software sees a byte -- so "listen to everything and decide from the data" works above the PHY and not below it. MeshCore and Meshtastic turn out to be identical on air except that one byte, and XPRS already sits on MeshCore's value by accident. Measured: joining Meshtastic costs 5.4x the airtime per packet, on the single channel Europe has. What the work would be, why the identity binding is cheap either way, and the one question left open. Status: evaluation, nothing implemented, no decision taken. |
...
| [f-droid.md](f-droid.md) | XPRS on F-Droid: the links (the submission thread, fdroiddata !48456, and the fork, pipelines and policies), what F-Droid is and why XPRS is there, how to read and answer the maintainers' feedback, the reproducible build and the release key, how to test a recipe on F-Droid's buildserver image, the recipe itself, how releases reach F-Droid, and the audit of every host and non-free piece the APK once had. |
...
| [diagrams/](diagrams/README.md) | **Every diagram lives here.** Mermaid, so they diff like text and cannot drift into a confident wrong answer. |
| [relay-gateway-flow.md](diagrams/relay-gateway-flow.md) | Relay, gateway and carrier drawn: the APRS lineage, which bearers are digipeated and by what, store-and-forward end to end, and the three places a packet crosses bearers. Names where the code and the specification disagree. |
| [message-receive.md](message-receive.md) | **Where a received 1:1 actually goes, and where it breaks.** The receive path for all four bearer inputs through the core to a rendered bubble, held against the code: a message keeps no stable identity across retries (measured: 40 messages became 394 packets with 394 identifiers), an XPRS 1:1 arriving over Reticulum is archived and never delivered, and the raw BLE lane acknowledges messages it never renders. Survey only — nothing fixed. Paired with [message-receive-flow.md](diagrams/message-receive-flow.md). |
| [transports-flow.md](diagrams/transports-flow.md) | The same flows drawn: component map, send sequence, per-bearer decision, receive demux + funnel, message lifecycle, and the bench instrument. Mermaid — renders on GitHub; `diagrams/transports-flow.html` is the same thing standalone in a browser. |
| [transports.md](transports.md) | The communication coordinator: one fan-out for every publish, the three answers a bearer can give, choosing one path under §36.0, the receive funnel that carries other people's mail, and the API for switching a bearer off to see what the station does about it. |
| [private-messages.md](private-messages.md) | Private and plain 1:1 messages: `x:` replaces `m:` so privacy is per packet and either side switches at any point, why a private send must refuse rather than downgrade, key discovery, splitting a sealed body, and how §36.0 picks the radio that carries it. |
