...
| [fdroid.md](fdroid.md) | Every network host the **built APK** can reach, audited from the binary rather than the source: what the store recipe must do (`--dart-define=SELF_UPDATE=false`), what was removed (Play Services, ipify, the GitHub downloads of the wasm runtime, the AI presets, mp4player's desktop binaries), what remains and why, and what is still open (wapps built from source). `tool/fdroid_scan.py` reproduces it. |
