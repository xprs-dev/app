...
| [transports-flow.md](transports-flow.md) | The same flows drawn: component map, send sequence, per-bearer decision, receive demux + funnel, message lifecycle, and the bench instrument. Mermaid — renders on GitHub; `transports-flow.html` is the same thing standalone in a browser. |
