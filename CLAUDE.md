...
checked out as a sibling) -- it is the ONE canonical copy; the app keeps no copy
of its own. `tool/xprs_corpus.py` regenerates `test/xprs_corpus.json` from it,
and the codec test checks against that corpus -- the whole value of the check is
that it tests against the specification document rather than against another
implementation. Edit the spec in `../spec`, then re-run the tool if a worked
packet changed.
