...
class XprsInlineAsm {
  XprsInlineAsm({this.maxBytes = kInlineMaxBytes, this.maxFiles = 32});
  ...
  void _sweep(DateTime at) =>
      _files.removeWhere((_, f) => at.difference(f.at) >= hold);
}

class _Inbound {
  _Inbound(this.size, this.ext, this.at) : _buf = Uint8List(size);
  final int size;
  final String ext;
  DateTime at;
